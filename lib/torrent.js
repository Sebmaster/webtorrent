...
Torrent.prototype._onParsedTorrent = function (parsedTorrent) {
  ...
  if (parsedTorrent.xs && !self.info) {
    ...
    var src = Array.isArray(parsedTorrent.xs) ? parsedTorrent.xs[0] : parsedTorrent.xs
    ...
    if (isHTTP) {
      ...
      this._xsRequest = get.concat(opts, function (err, res, torrent) {
        if (err) {
          self._debug('received error "%s" while downloading exact source', err.message)
          return
        } else if (res.statusCode !== 200) {
          self._debug('received status code %s while downloading exact source', res.statusCode)
          return
        }
        ...
        if (parsedTorrent) {
          if (parsedTorrent.infoHash !== self.infoHash) {
            self._debug('xs info hash does not match')
            return
          }
